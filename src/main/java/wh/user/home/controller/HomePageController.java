...
public class HomePageController implements ApplicationContextAware {
	...
	public void searchNangbuAjax(@RequestParam("ingredients[]") int ingredients[], HttpServletResponse response)
			throws IOException {
		List<HomePageRecipeDto> list = homePageService.searchFromIngre(ingredients);
	...
	public void confirmRecipe(int recipe_id, HttpServletResponse response, HttpSession session) throws IOException {
		...
		HomePageMemberDto mem = homePageService.getMember(member_id);
		...
		if (confirm == null) {
			// 포크가 있을때
			if (mem.getFork() > 0) {
				confirm = new HomePageRecipeConfirmDto();
